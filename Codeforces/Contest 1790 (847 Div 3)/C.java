...
public class C {
    ...
    public static void solve() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < n - 1; j++) {
                int tempOfTemp = in.nextInt();
                // System.out.print(tempOfTemp);
                temp.add(tempOfTemp);
            }

            if (hm.containsKey(temp.get(0)) == true) {
                hm.put(temp.get(0), (hm.get(temp.get(0)) + 1));
            } else {
                hm.put(temp.get(0), 1);
            }
            arr.add(temp);
        ...
        for (ArrayList<Integer> vaList : arr) {
            if (vaList.get(0) == min) {
