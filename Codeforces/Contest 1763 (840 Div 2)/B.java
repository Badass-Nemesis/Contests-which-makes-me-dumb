...
public class B {
    ...
    public static void solve() throws IOException {
        ...
        for (int i = 0; i < n; i++) {
            Pair temp = new Pair(power[i], health[i]);
            max = Math.max(power[i], max);
            pq.add(temp);
        }

        // pq is not arranging in increasing order 😑. So code is not working
        int tempK = k;
        int count = 0;
        while (count < n) {
            if (k < 1) {
            ...
            for (Pair p : pq) {
                System.out.println(p.pow + " " + p.heal);
                if (k < 1) {
                    System.out.println("NO");
                    return;
                }
    ...
    public static class Pair implements Comparable<Pair> {
        ...
        public Pair(long pow, long heal) {
            this.pow = pow;
            this.heal = heal;
        }

        public int compareTo(Pair other) {
            return (int) (this.pow - other.pow);
        }
    }
