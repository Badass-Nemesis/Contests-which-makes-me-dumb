...
public class A {
    ...
    public static void solve() throws IOException {
        ...
        int oddWala = Integer.MAX_VALUE;
        int evenWala = Integer.MAX_VALUE;
        for (int i = 0; i < n; i++) {
            if (arr[i] % 2 != 0) {
                oddWala = Math.min(oddWala, countReturn(arr[i]));
            } else {
                evenWala = Math.min(evenWala, countReturn(arr[i]));
            }
        }

        System.out.println(Math.min(oddWala, evenWala));
    }
    ...
    static boolean isPrimeOdd(long n) {
        for (int i = 2; i <= n * n; i++) {
            if (n % i == 0) {
