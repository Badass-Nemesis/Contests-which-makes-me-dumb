...
public class E {
    ...
    public static int dfs(ArrayList<ArrayList<Integer>> adj, int node, int levels) {
        if (node == levels) {
            return 1;
        }
        ...
        for (Integer neighbours : adj.get(node)) {
            dpArr[neighbours] = dfs(adj, neighbours, levels);
            sum += dpArr[neighbours];
        }

        return dpArr[node] = sum;
    }
    ...
    public static void solve() throws IOException {
        ...
        dpArr = new int[levels + 1];
        Arrays.fill(dpArr, -1);
        int count = dfs(adj, 1, levels);
        System.out.println(count);
    }
